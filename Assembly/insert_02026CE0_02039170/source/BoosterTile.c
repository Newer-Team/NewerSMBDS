...
{   
    ...
    if (athis->currentMarioAnim == 0x22) 
        tiletimer = 0;
    
    if ((tiletimer) && (*Frozen == 0)) 
        tiletimer--;
    
    if ((athis->IsMarioGroundPounding77E & 0x2) == 0 && 
        athis->currentMarioAnim != 0x11 && 
        athis->currentMarioAnim != 0xD && 
        athis->currentMarioAnim != 0xC)
    {
        ...
        if ((tiletimer<20) && (tiletimer))
        {
            if (athis->xVelocity > 0) 
                athis->xVelocity+=0x100; 
            
            (*((u32*)(((u32*)(athis->unk6BC))+3))) -=0x100;
        }
    }
    
    if (abs(athis->xVelocity) <= 0x3000) 
        *((u32*)(((u32*)(athis->unk6BC))+3)) = 0x3000;
}
