...
class SDAT:
    """
    SDAT is the archive file that contains all of the other stuff.
    """
    def __init__(self, data=None):
        ...
        self.firstFilePadAmount = None
        self.filePadAmount = 0x20
    ...
    def _initFromData(self, data):
        ...
        for i in range(fatCount):
            ...
            if fileOffset % 0x200 == 0x100:
                self.filePadAmount = min(self.filePadAmount, 0x100)
            if fileOffset % 0x100 == 0x80:
                self.filePadAmount = min(self.filePadAmount, 0x80)
            if fileOffset % 0x80 == 0x40:
                self.filePadAmount = min(self.filePadAmount, 0x40)
            if fileOffset % 0x40 == 0x20:
                self.filePadAmount = min(self.filePadAmount, 0x20)
            if fileOffset % 0x20 == 0x10:
                self.filePadAmount = min(self.filePadAmount, 0x10)
            if fileOffset % 0x10 == 8:
                self.filePadAmount = min(self.filePadAmount, 8)
            if fileOffset % 8 == 4:
                self.filePadAmount = min(self.filePadAmount, 4)
            if fileOffset % 4 == 2:
                self.filePadAmount = min(self.filePadAmount, 2)
            if fileOffset % 2 == 1: # yes, this happens sometimes
                self.filePadAmount = min(self.filePadAmount, 1)

            if i == 0:
                self.firstFilePadAmount = self.filePadAmount

            file = data[fileOffset : fileOffset + fileSize]
            files.append(file)

        if self.firstFilePadAmount == self.filePadAmount:
            self.firstFilePadAmount = None

        # Check if the end is definitely unpadded (that is, if there
        # should be padding and it's not present)
        if finalFileEnd == len(data) and finalFileEnd % self.filePadAmount != 0:
            self.padAtEnd = False
        ...
        for entryOff, symb in itertools.zip_longest(getInfoEntryOffsets(0),
            ...
            if entryOff is None:
                ...
                sseq = soundSequence.SSEQ(files[fileID], unk02, bankID,
                            volume, channelPressure, polyphonicPressure, play)
        ...
        for entryOff, symb in itertools.zip_longest(getInfoEntryOffsets(7),
            ...
            if entryOff is None:
                ...
                fileID, unk02, volume, priority, play, unk07 = \
                    struct.unpack_from('<HH4B', data, entryOff)
                strm = soundStream.STRM(files[fileID], unk02, volume, priority, play, unk07)
    ...
    def save(self):
            ...
            if not nested:
                ...
                for symbol, entry in namedList:
                    if symbol is not None:
                        anyActualSymbols = True
                        break
                    for subSymbol in entry.sequenceNames():
        ...
        for i, (_, sseq) in enumerate(self.sequences):
            if sseq is None:
                ...
                (file, unk02, bankID, volume, channelPressure,
                    polyphonicPressure, play) = sseq.save()
        ...
        for i, (_, strm) in enumerate(self.streams):
            if strm is None:
                ...
                file, unk02, volume, priority, play, unk07 = strm.save()
        ...
        if self.firstFilePadAmount is not None:
            while len(data) % self.firstFilePadAmount:
                data.append(0)
        ...
        for i, file in enumerate(files):
            ...
            while len(data) % self.filePadAmount:
                data.append(0)
            ...
            if self.fatLengthsIncludePadding:
                while fLen % self.filePadAmount: fLen += 1
