...
class NintendoDSRom:
    """
    A class that represents a Nintendo DS ROM file (.nds).
    """
    ...
    def _initAsNew(self):
        ...
        self.secureTransferTimeout = 0x0D7E
        self.arm9AutoLoadAddress = 0x02000AAC
        self.arm7AutoLoadAddress = 0x02380158
    ...
    def _initFromData(self, data):
        ...
        self.gameTitle = readRaw(12).rstrip(b'\0')
        self.gameCode = readRaw(4)
        self.makerCode = readRaw(2)
        self.unitCode = read8()
        ...
        self.secureAreaChecksum = read16() # TODO: Actually recalculate
                                           # this upon saving.
        self.secureTransferTimeout = read16()
        assert headerOffset == 0x70
        self.arm9AutoLoadAddress = read32()
        self.arm7AutoLoadAddress = read32()
    ...
    def save(self):
        """
        Save this ROM back to a bytes object.
        """
        ...
        assert headerOffset == 0
        writeRaw(self.gameTitle.ljust(12, b'\0'))
        writeRaw(self.gameCode)
        writeRaw(self.makerCode)
        ...
        write16(self.secureTransferTimeout)
        assert headerOffset == 0x70
        write32(self.arm9AutoLoadAddress)
        write32(self.arm7AutoLoadAddress)
        writeRaw(self.secureAreaDisable.ljust(8, b'\0'))
        assert headerOffset == 0x80
    ...
    def __str__(self):
        """
        Make a nice string representation of the ROM object.
        """
        title = repr(bytes(self.gameTitle))[2:-1].rstrip(' ')
        code = repr(bytes(self.gameCode))[2:-1]
        return f'<NDS ROM: "{title}" ({code})>'
