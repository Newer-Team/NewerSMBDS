...
def convertDictToFilenamesTable(d):
    ...
    for folderName, folderDict in d.get('folders', {}).items():
        thisFolder.folders[folderName] = convertDictToFilenamesTable(folderDict)
    return thisFolder
...
def jsonToFnt(jsonData):
    ...
    table.
    """
    j = json.loads(jsonData, object_pairs_hook=collections.OrderedDict)
    table = convertDictToFilenamesTable(j)
    return table