...
bool CheckIfMarioIsInRange(Tripleblock* trpl)
	{
		...
		if (xoffs > Distance || xoffs < -Distance || yoffs < -0x100000 || yoffs > 0x300000)  return false;
		...
		if (!trpl->hit)
			{
				...
				if (!(trpl->animtimer % 32)) 
					{
						if (trpl->timeshit) trpl->timeshit++;
					}

				if (trpl->animtimer == 64) trpl->animtimer = 0;
...
void CheckIfMario(Tripleblock* trpl, MarioActor* Player)
	{
		...
		if ((u32)Player == (u32)getPtrToPlayerActor()) Tripleblock_SetBlockStateHit(trpl);
		else return;
	}
...
void CheckIfShell(Tripleblock* trpl, EnemyClassObject* Enemy)
	{
		...
		else Tripleblock_SetBlockStateHit(trpl);
	}	
		...
		if ((!((trpl->spriteData >> 20) & 1)) || (trpl->timeshit > 8))
			{
				...
				if (trpl->ModelFile == 2537) getTexturePalBase(Texture, 0, &trpl->Tex.palbase);
				else getTexturePalBase(Texture, 1, &trpl->Tex.palbase);
			}
		...
		for (int i = 0; i < 3; i++)
			{
				...
				if (((trpl->spriteData >> (4*i)) & 0xF) == 2) PlaySNDEffect(SE_OBJ_GET_COIN, &Position);
				else PlaySNDEffect(SE_OBJ_ITEM_APPEAR, &Position);
				Position.x -= 0x10000;
			}
	}
...
void Tripleblock_StateBeingHit(Tripleblock* trpl)
	{
		...
		if (trpl->movetimer < 6) trpl->position.y += (0x1000 * gpa); 
		else trpl->position.y -= (0x1000 * gpa);
		...
		if (trpl->movetimer == 10) 
			{
				...
				if (((trpl->spriteData >> 20) & 1) && (trpl->timeshit < 10)) trpl->hit = false;
				if ((!((trpl->spriteData >> 20) & 1)) || (trpl->timeshit > 9)) UnregisterActivePhysics(&trpl->activephysics);
			}
	}
...
void Tripleblock_Megashroom(Tripleblock* trpl)
	{
		...
		if (trpl->hit) spritedata = 0x600000;
