...
int Titlescreen_loadFiles()
{
    ...
    if (TitleScrNumber > 8) 
        World = TitleScrNumber - 10;            // 10 is added during the save check when the secret music is supposed to trigger.
...
void Titlescreen_Start(Titlescreen* tscr)   // State 1
{
    ...
    if (tscr->ScreenBrightness != 0) 
        setBrightness(3, tscr->ScreenBrightness+=1);
...
void Titlescreen_MoveOut(Titlescreen* tscr)     // State 2
{
    ...
    if (tscr->CopyrightLogoFade!=0) 
        tscr->CopyrightLogoFade--;
