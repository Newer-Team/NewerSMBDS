		...
		if (((*((*PtrToBlock1))>>16)&0xF) == 2)								//Fixes Mini Mushroom jump height in space.
			{
				if (Player->marioPowerupState == 4)
					{
						if (*PlayerID == 0) *JumpHeight = 0x4280;
						else *JumpHeight = 0x4610;
					}
				else
					{
						if (*PlayerID == 0) *JumpHeight = 0x3A80;
						else *JumpHeight = 0x4010;
						...
						if (ticker < 240)
							{
								...
								if (RNG(20) < 2) Rumble(1);

								if (Player->xAccel > 0) storedpos = Player->xPos;
								...
								if ((!(pressedKeys & DOWN)) || (((!Player->isOnGround)) && !(Player->IsMarioGroundPounding77E&2)) || (Player->marioPowerupState == 3))
									{
										if (ticker<=32) Player->xPos = Player->xPos+(0xC60+(ticker*0x80));
										else if (ticker>=208) Player->xPos = Player->xPos+(0x2560-(ticker*16));
										else  Player->xPos = Player->xPos+0x2560;
										storedpos = Player->xPos;
									}
...
void hook_020A2ED0()												// Hook in Stage Draw. Darkness setting.
	{
		...
		if ((Unk2 == 5) ||( Unk2 == 7))						// Lightning
			{
				if (lightningtimer) lightningtimer--;
				...
				if ((lightningtimer == 0) && !(*MenuOpen))  setBrightness(1,-getUnk1ForView(CurrentView));
				
				if ((!*MenuOpen) && (Frozen == 0)) PlaySNDEffect2(SE_PLY_KISS, 0);			// Rain.
			}
				...
				if (!(*MenuOpen))
					{
					  ...
					  // Attempt to darken due to forward progress
						...
						if (((u8)REG_MASTER_BRIGHT) > currentW8TowerBrightness) setBrightness(1,currentW8TowerBrightness);
					}
			}
		return;
	}
